...
SUB_DIRS += Menu
SUB_DIRS += Src
SUB_DIRS += STM32_USB-FS-Device_Driver
SUB_DIRS += STM32F10x_StdPeriph_Driver
#SUB_DIRS += driver
#SUB_DIRS += app

ifeq ($(USE_UCOS),y)
SUB_DIRS +=  kernel/ucos
endif
...
LDFLAGS = $(CPU_SPEC) -lc -lm -lnosys -specs=nano.specs -Wl,-Map=$(TARGET).map,--cref -u _printf_float -u _scanf_float -static
